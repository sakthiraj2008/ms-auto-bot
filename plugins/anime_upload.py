...
#                       admin dashboard
#   /upload_episode  — pick type -> pick title -> pick an existing season
#                       (shown with its current episode count) or start a
#                       new one -> confirm screen with an Add Episode button
#                       -> send episode title -> forward ONE file ->
#                       auto-generates the Fast Download URL and inserts it
#                       as the next episode of that season (episode number
#                       is computed automatically, you never type it)
...
#   /add_content     — pick type -> title -> IMDb rating -> poster upload ->
...
try:
    ...
    # config.py on the server hasn't been updated with this constant yet —
    # fall back to the default bucket name instead of crashing the bot.
...
from plugins.supabase_client import (
    is_configured, sb_select, sb_insert, sb_update, sb_count, sb_auth_login,
)
try:
    from plugins.supabase_client import sb_storage_upload
except ImportError:
    # plugins/supabase_client.py on the server hasn't been updated with this
    # function yet — /add_content's poster/banner upload step will show a
    # clear error instead of the whole bot failing to start.
    def sb_storage_upload(*args, **kwargs):
        raise RuntimeError(
            "sb_storage_upload isn't available — plugins/supabase_client.py on "
            "the server is out of date. Redeploy the full updated file."
        )
...
async def _fetch_season_counts(content_id: str) -> dict:
    """{season_number: episode_count} for this title, sorted by season."""
    rows = sb_select("episodes", {"content_id": f"eq.{content_id}", "select": "season_number"})
    counts = {}
    for row in rows:
        n = row.get("season_number")
        if n is not None:
            counts[n] = counts.get(n, 0) + 1
    return dict(sorted(counts.items()))


def _existing_season_keyboard(prefix: str, counts: dict):
    """Seasons that already have episodes, each showing its episode count,
    plus a button to start a season that doesn't exist yet."""
    rows = [
        [InlineKeyboardButton(f"Season {n} — {c} ep", callback_data=f"{prefix}_season_pick:{n}")]
        for n, c in counts.items()
    ]
    rows.append([InlineKeyboardButton("➕ New Season", callback_data=f"{prefix}_new_season")])
    rows.append([InlineKeyboardButton("✖️ Cancel", callback_data=f"{prefix}_cancel")])
    return InlineKeyboardMarkup(rows)


def _confirm_season_keyboard(prefix: str):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("➕ Add Episode", callback_data=f"{prefix}_addep")],
        [InlineKeyboardButton("🔙 Back to seasons", callback_data=f"{prefix}_back_seasons")],
        [InlineKeyboardButton("✖️ Cancel", callback_data=f"{prefix}_cancel")],
    ])


async def _show_season_confirm(query, user_id, season):
    session = SESSIONS[user_id]
    session["season"] = season
    session["step"] = "confirm_season"
    counts = await _fetch_season_counts(session["content_id"])
    count = counts.get(season, 0)
    await query.message.edit(
        f"📺 <b>{session['content_title']}</b> — Season {season}, currently {count} episode(s).",
        reply_markup=_confirm_season_keyboard("ue"),
    )
...
async def cb_upload_episode(client, query):
    ...
    if data.startswith("ue_content:"):
        ...
        counts = await _fetch_season_counts(content_id)
        if not counts:
            SESSIONS[user_id]["step"] = "pick_new_season"
            return await query.message.edit(
                f"✅ <b>{content[0]['title']}</b> selected. No seasons yet.\n\n🔢 Pick the season number:",
                reply_markup=_season_keyboard("ue"),
            )
        return await query.message.edit(
            f"✅ <b>{content[0]['title']}</b> selected.\n\n📺 Pick a season:",
            reply_markup=_existing_season_keyboard("ue", counts),
        )

    if data == "ue_new_season":
        SESSIONS[user_id]["step"] = "pick_new_season"
        return await query.message.edit("🔢 Pick the season number:", reply_markup=_season_keyboard("ue"))

    if data == "ue_back_seasons":
        content_id = SESSIONS[user_id]["content_id"]
        counts = await _fetch_season_counts(content_id)
        SESSIONS[user_id]["step"] = "select_season"
        if not counts:
            SESSIONS[user_id]["step"] = "pick_new_season"
            return await query.message.edit("No seasons yet.\n\n🔢 Pick the season number:", reply_markup=_season_keyboard("ue"))
        return await query.message.edit("📺 Pick a season:", reply_markup=_existing_season_keyboard("ue", counts))

    if data.startswith("ue_season_pick:"):
        season = int(data.split(":", 1)[1])
        return await _show_season_confirm(query, user_id, season)
    ...
    if data == "ue_addep":
        ...
        season = SESSIONS[user_id]["season"]
...
# ============================================================
# /add_content
# type -> title -> imdb rating -> poster photo -> banner photo -> year ->
    ...
    if data == "ac_genre_done":
        ...
        try:
            row = {
                "type": session["type"],
                "title": session["title"],
                "imdb": None if session.get("imdb_rating") is None else str(session["imdb_rating"]),
...
async def handle_incoming_text(client, message):
    ...
    if session.get("cmd") == "add_content":
        ...
        if step == "await_title":
            ...
            return await message.reply("⭐ Send the <b>IMDb rating</b> (e.g. 8.5), or send - to skip:")

        if step == "await_imdb":
            rating_text = message.text.strip()
            if rating_text == "-":
                session["imdb_rating"] = None
            else:
                try:
                    ...
                    return await message.reply("Please send a number between 0 and 10 (e.g. 8.5), or - to skip.")
                session["imdb_rating"] = rating
