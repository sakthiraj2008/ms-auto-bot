...
#                       admin dashboard
#   /upload_episode  — pick type -> pick title -> forward ONE file ->
#                       auto-generates the Fast Download URL and
#                       inserts it as the next episode
...
from plugins.supabase_client import is_configured, sb_select, sb_insert, sb_update, sb_count
...
TYPE_LABELS = dict(CONTENT_TYPES)
PAGE_SIZE = 8
...
async def _next_episode_number(content_id: str) -> int:
    rows = sb_select(
        "episodes",
        {"content_id": f"eq.{content_id}", "select": "episode_number", "order": "episode_number.desc", "limit": "1"},
    )
...
async def cb_upload_episode(client, query):
    ...
    if data.startswith("ue_content:"):
        ...
        await query.message.edit(
            f"✅ <b>{content[0]['title']}</b> selected.\n\n📤 Now forward the episode video file here."
        )
...
async def handle_incoming_file(client, message):
    ...
    if session["cmd"] == "upload_episode":
        try:
            ep_number = await _next_episode_number(session["content_id"])
            sb_insert(
                "episodes",
                {
            ...
            await status.edit(
                f"✅ Updated — <b>{session['content_title']}</b> Episode {ep_number} added."
            )
        except Exception as e:
...
async def handle_incoming_text(client, message):
    session = SESSIONS.get(message.from_user.id)
    if not session or session.get("step") != "await_title":
