...
def sb_insert(table: str, row: dict):
    ...
    Raises ValueError with Supabase's actual error message on failure (column
    name/type mismatches, constraint violations, etc.) instead of a bare
    '400 Bad Request' that hides the real cause."""
    url = f"{SUPABASE_URL.rstrip('/')}/rest/v1/{table}"
    resp = requests.post(url, headers=_base_headers(prefer="return=representation"), json=row, timeout=15)
    if not resp.ok:
        raise ValueError(_extract_postgrest_error(resp))
    data = resp.json()
    return data[0] if isinstance(data, list) and data else data


def _extract_postgrest_error(resp) -> str:
    try:
        body = resp.json()
        return body.get("message") or body.get("hint") or body.get("details") or resp.text
    except Exception:
